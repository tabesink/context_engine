...
class JobService:
    ...
    def enqueue_index_document(self, *, document_id: str) -> str:
        job = self.jobs.create(kind="index_document", document_id=document_id)
        if self.run_inline:
            self.run_index_job(job.id)
            return job.id

        from app.workers.tasks import run_index_job

        queued_job = self._queue().enqueue(run_index_job, job.id)
    ...
    def enqueue_navigation_process_document(self, *, document_id: str) -> str:
        job = self.jobs.create(kind="navigation_process_document", document_id=document_id)
        if self.run_inline:
            self.run_navigation_process_job(job.id)
            return job.id

        from app.workers.tasks import run_navigation_process_job

        queued_job = self._queue().enqueue(run_navigation_process_job, job.id)
        metadata = job.meta | {"rq_job_id": queued_job.id}
        self.jobs.set_status(job, JobStatus.QUEUED, metadata=metadata)
        return job.id
    ...
    def run_navigation_process_job(self, job_id: str) -> None:
        from app.workers.tasks import run_navigation_process_job

        run_navigation_process_job(job_id)
