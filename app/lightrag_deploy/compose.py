...
def _append_provider_env(
    ...
    llm_api_key = None
    if llm and llm.api_key_env_var and provider_secrets:
        llm_api_key = provider_secrets.get(llm.api_key_env_var)
    if not llm_api_key:
    ...
    _append_env(lines, "LLM_BINDING", llm.binding if llm else settings.llm_binding)
    _append_env(lines, "LLM_BINDING_HOST", llm.base_url if llm else settings.llm_binding_host)
    _append_env(lines, "LLM_BINDING_API_KEY", llm_api_key)
    _append_env(lines, "LLM_MODEL", llm.model if llm else settings.llm_model)
...
def write_domain_env(
    ...
    paths.env_file.write_text(
        render_domain_env(domain, settings, provider_secrets, llm=llm),
        encoding="utf-8",
    )
...
def render_domain_env(
    ...
    *,
    ...
    _append_provider_env(lines, domain, settings, provider_secrets, llm=llm)
    return "\n".join(lines) + "\n"
