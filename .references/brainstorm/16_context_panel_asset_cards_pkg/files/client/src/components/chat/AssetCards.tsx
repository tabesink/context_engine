...
function AuthenticatedAssetImage({ src, alt, className }: { src?: string | null; alt: string; className?: string }) {
  ...
  return <img src={objectUrl} alt={alt} className={cn("mx-auto h-auto w-full object-contain", className)} />;
}
...
function classifyAsset(assetType?: string | null, mimeType?: string | null): AssetCardKind {
...
function parseMarkdownTable(value?: string | null): string[][] {
  if (!value) return [];
  const lines = value
    .split("\n")
    .map((line) => line.trim())
