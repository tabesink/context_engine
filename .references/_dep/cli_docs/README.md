...
LightRAG domain administration is TUI-first for human operators and REST-first for automation. Domain deployment requires backend settings such as **`LIGHTRAG_DEPLOY_ENABLED=true`** plus the Docker/image/storage variables documented in **`.env.lightrag-deploy.example`**. Runtime LightRAG uses the single **`LIGHTRAG_DOMAIN_REGISTRY`** file; clients choose a registered `lightrag_domain_id` before upload, retrieval, graph, or workspace-tree calls.
