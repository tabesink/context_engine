...
def create_app(args):
    ...
    if args.llm_binding not in [
        ...
        "azure_openai",
        "openai-ollama",
        "bedrock",          # NOTE: bedrock-ollama special case --> set to args.llm_binding=bedrock in config.py
    ]:
    ...
    async def lifespan(app: FastAPI):
        ...
        try:
            ...
            if args.document_loading_engine == "DOCLING":
                logger.info("Running Docling startup preflight")
                ready = await ensure_docling_ready(
                    install_if_missing=True, context="startup"
                )
                if not ready:
                    logger.warning(
                        "Docling startup preflight failed. Uploads may fall back to flat extraction."
                    )
            else:
                logger.info(
                    "Skipping Docling startup preflight; DOCUMENT_LOADING_ENGINE is not DOCLING."
                )
    ...
    if args.llm_binding == "bedrock" or args.llm_binding_host == "bedrock-ollama":
        from lightrag.llm.bedrock import bedrock_model_complete
        if args.embedding_binding == "ollama":
            from lightrag.llm.ollama import ollama_embed
        elif args.embedding_binding == "bedrock":
    ...
    if args.llm_binding in ["lollms", "ollama", "openai"]:
        rag = LightRAG(
            ...
            llm_model_name=args.llm_model,    # NOTE: bedrock model name set in .env file
        ...
        rag = LightRAG(
            working_dir=args.working_dir,
            workspace=args.workspace,
            llm_model_func=bedrock_model_complete,
            chunk_token_size=int(args.chunk_size),
            chunk_overlap_token_size=int(args.chunk_overlap_size),
            llm_model_kwargs={},
    ...
    @app.post("/shutdown", dependencies=[Depends(combined_auth)])
