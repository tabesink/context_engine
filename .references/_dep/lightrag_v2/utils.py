...
# Prefer loading .env from the workspace root (outside server folder),
# fallback to server folder or auto-discovery. OS env vars still take precedence.
try:
    current_file_path = Path(__file__).resolve()
    candidate_paths = [
        current_file_path.parents[2] / ".env",  # workspace root /.env
        current_file_path.parents[1] / ".env",  # server_v0.1.0 /.env (fallback)
    ]
