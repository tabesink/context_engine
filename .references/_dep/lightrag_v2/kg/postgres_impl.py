...
class PostgreSQLDB:
    ...
    async def _migrate_llm_cache_add_mode(self):
        """Add mode column and conflict target support for LLM cache if missing."""
        try:
            check_column_sql = """
            SELECT column_name
            FROM information_schema.columns
            ...
            column_info = await self.query(check_column_sql)
            if not column_info:
                logger.info("Adding mode column to LIGHTRAG_LLM_CACHE table")
                await self.execute(
                    """
                    ALTER TABLE LIGHTRAG_LLM_CACHE
                    ADD COLUMN mode VARCHAR(32) NULL DEFAULT 'default'
                    """
                )
                await self.execute(
                    """
                ...
                logger.info("mode column already exists in LIGHTRAG_LLM_CACHE table")

            check_index_sql = """
            SELECT 1
            FROM pg_indexes
            WHERE tablename = 'lightrag_llm_cache'
            AND indexname = 'idx_lightrag_llm_cache_workspace_mode_id'
            """
            index_info = await self.query(check_index_sql)
            if not index_info:
                logger.info(
                    "Creating workspace/mode/id unique index on LIGHTRAG_LLM_CACHE"
                )
                await self.execute(
                    """
                    CREATE UNIQUE INDEX idx_lightrag_llm_cache_workspace_mode_id
                    ON LIGHTRAG_LLM_CACHE(workspace, mode, id)
                    """
                )
        except Exception as e:
            logger.warning(f"Failed to add mode support to LIGHTRAG_LLM_CACHE: {e}")
    ...
    async def _migrate_structural_chunking_columns(self):
        """Add structural chunking metadata columns used by Phase 4.

        CUSTOM STRUCTURAL CHUNKING:
        Existing PostgreSQL installs may already have the LightRAG tables. These
        columns are additive and optional so flat chunks continue to work, while
        structural chunks can persist page/section/artifact citation data.
        """
        columns = {
            "LIGHTRAG_DOC_STATUS": {
                "content": "TEXT NULL",
        ...
        for table_name, table_columns in columns.items():
            for column_name, column_type in table_columns.items():
                ...
                column_info = await self.query(
                    check_column_sql,
                    {"table_name": table_name.lower(), "column_name": column_name},
                )
                if not column_info:
                    await self.execute(
    ...
    async def check_tables(self):
        ...
        # Migrate LLM cache table to add mode field/conflict target if needed
        try:
            await self._migrate_llm_cache_add_mode()
        except Exception as e:
            logger.error(f"PostgreSQL, Failed to migrate LLM cache mode field: {e}")
            # Don't throw an exception, allow the initialization process to continue
        ...
        # CUSTOM STRUCTURAL CHUNKING:
        # Add optional doc/chunk metadata columns after base tables exist. This
        # keeps clean-slate and existing local databases on the same schema.
        try:
            await self._migrate_structural_chunking_columns()
        except Exception as e:
            logger.error(
                f"PostgreSQL, Failed to migrate structural chunking fields: {e}"
            )
        ...
        if is_namespace(self.namespace, NameSpace.KV_STORE_TEXT_CHUNKS):
            ...
            for k, v in data.items():
                ...
                _data = {
                    ...
                    # CUSTOM STRUCTURAL CHUNKING:
                    # Text chunk storage keeps the full citation payload because
                    # graph/entity retrieval loads chunks from this KV namespace.
                    "page_start": v.get("page_start"),
                    "page_end": v.get("page_end"),
                    "section_path": v.get("section_path"),
                    "chunk_type": v.get("chunk_type"),
                    "artifact_manifest_path": v.get("artifact_manifest_path"),
                    "chunk_metadata": json.dumps(v.get("chunk_metadata", {})),
    ...
    def _upsert_chunks(
        ...
        try:
            ...
            data: dict[str, Any] = {
                ...
                # CUSTOM STRUCTURAL CHUNKING:
                # Vector storage stores compact citation fields so vector-only
                # retrieval can return the same source metadata as graph modes.
                "page_start": item.get("page_start"),
                "page_end": item.get("page_end"),
                "section_path": item.get("section_path"),
                "chunk_type": item.get("chunk_type"),
                "artifact_manifest_path": item.get("artifact_manifest_path"),
                "chunk_metadata": json.dumps(item.get("chunk_metadata", {})),
                "create_time": current_time,
                "update_time": current_time,
            }
        except Exception as e:
...
class PGDocStatusStorage(DocStatusStorage):
    ...
    async def upsert(self, data: dict[str, dict[str, Any]]) -> None:
        ...
        # CUSTOM STRUCTURAL CHUNKING:
        # `metadata` stores the compact structural summary and manifest pointer
        # for Docling documents. The full structure still lives in the artifact
        # manifest, but this column lets queued documents round-trip through
        # PostgreSQL before they are processed into chunks.
        # Modified SQL to include created_at, updated_at, chunks_list, and metadata in both INSERT and UPDATE operations
        # All fields are updated from the input data in both INSERT and UPDATE cases
        sql = """insert into LIGHTRAG_DOC_STATUS(workspace,id,content,content_summary,content_length,chunks_count,status,file_path,chunks_list,metadata,created_at,updated_at)
...
SQL_TEMPLATES = {
    ...
    "get_by_id_text_chunks": """SELECT id, tokens, COALESCE(content, '') as content,
                                ...
                                COALESCE(chunk_metadata, '{}'::jsonb) as chunk_metadata,
    ...
    "upsert_text_chunk": """INSERT INTO LIGHTRAG_DOC_CHUNKS (workspace, id, tokens,
                      ...
                      page_start, page_end, section_path, chunk_type, artifact_manifest_path,
                      chunk_metadata, create_time, update_time)
    ...
    "chunks": """
        ...
        SELECT id, tokens, chunk_order_index, full_doc_id, content, file_path,
               page_start, page_end, section_path, chunk_type,
               artifact_manifest_path, chunk_metadata,
               distance, EXTRACT(EPOCH FROM create_time)::BIGINT as created_at FROM
            (
                SELECT id, tokens, chunk_order_index, full_doc_id, content, file_path,
                       page_start, page_end, section_path, chunk_type,
                       artifact_manifest_path, chunk_metadata, create_time,
