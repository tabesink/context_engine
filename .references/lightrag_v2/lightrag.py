...
from .structural_chunking import chunking_by_doc_structure
...
class LightRAG:
    ...
    def __post_init__(self):
        ...
        self.chunks_vdb: BaseVectorStorage = self.vector_db_storage_cls(  # type: ignore
            ...
            # Vector backends only return fields listed here. These citation
            # fields let vector retrieval return the same page/section/artifact
            # shape as graph-backed retrieval without loading text chunk storage.
            meta_fields={
                "full_doc_id",
                "content",
                "file_path",
    ...
    async def apipeline_enqueue_documents(
        ...
        file_paths: str | list[str] | None = None,
        document_metadata: dict[str, Any] | list[dict[str, Any] | None] | None = None,
        ...
        Args:
            ...
            ids: list of unique document IDs, if not provided, MD5 hash IDs will be generated
            file_paths: list of file paths corresponding to each document, used for citation
            document_metadata: Optional compact metadata from structural extraction.
            overwrite_existing: If True, replace existing records for matching IDs.
        """
        ...
        # CUSTOM STRUCTURAL CHUNKING:
        # Metadata travels with the pending doc-status record because later
        # processing may happen in another worker pass. Text-only callers do not
        # need to pass this argument, so the legacy API remains compatible.
        if document_metadata is None:
            document_metadata = [None] * len(input)
        elif len(document_metadata) != len(input):
            raise ValueError("Number of document metadata entries must match documents")
        ...
        if ids is not None:
            ...
            contents = {
                id_: {"content": doc, "file_path": path, "metadata": metadata or {}}
                for id_, doc, path, metadata in zip(
                    ids, input, file_paths, document_metadata
                )
            }
        else:
            # Clean input text and remove duplicates
            cleaned_input = [
                (clean_text(doc), path, metadata or {})
                for doc, path, metadata in zip(input, file_paths, document_metadata)
            ]
        ...
        for id_, content_data in contents.items():
            ...
            dedupe_key = content if not metadata else f"{id_}:{content}"
            if dedupe_key not in unique_contents:
                unique_contents[dedupe_key] = (id_, content, file_path, metadata)

        # Reconstruct contents with unique content
        contents = {
            id_: {"content": content, "file_path": file_path, "metadata": metadata}
            for _, (id_, content, file_path, metadata) in unique_contents.items()
        }

        # CUSTOM STRUCTURAL CHUNKING:
        # Reingest is deterministic for clean-slate structural builds. If the
        # caller supplies an explicit document ID and asks to overwrite, delete
        # old chunks/doc-status first so the new artifact manifest and chunk list
        # become the single source of truth for that document.
        if overwrite_existing and ids is not None:
            for doc_id in ids:
                ...
                async def process_document(
                    ...
                    async with semaphore:
                        ...
                        try:
                            ...
                            # CUSTOM STRUCTURAL CHUNKING:
                            # If Docling metadata is present, generate chunks
                            # from normalized structure instead of the flat token
                            # window. Chunk IDs include structural identity so
                            # repeated text in different sections/pages remains
                            # separately citable. If metadata is missing or empty,
                            # the original token chunker remains the fallback.
                            structural_metadata = status_doc.metadata or {}
                            structural_dps = []
                            if structural_metadata.get("source_engine") == "docling":
                                structural_blocks = (
                                    (structural_metadata.get("structure") or {}).get(
                                        "blocks"
                                    )
                                    or []
                                )
                                async with pipeline_status_lock:
                                    log_message = (
                                        "Structure-aware chunking started for "
                                        f"{file_path}: {len(structural_blocks)} blocks"
                                    )
                                    logger.info(log_message)
                                    pipeline_status["latest_message"] = log_message
                                ...
                                structural_dps = chunking_by_doc_structure(
                                    self.tokenizer,
                                    status_doc.content,
                                    structural_metadata,
                                ...
                                if structural_dps:
                                    log_message = (
                                        "Structure-aware chunking completed for "
                                        f"{file_path}: {len(structural_dps)} chunks"
                                    )
                                else:
                                    log_message = (
                                        "Structure-aware chunking produced no chunks "
                                        f"for {file_path}; falling back to token chunking"
                                    )
                                async with pipeline_status_lock:
                                    logger.info(log_message)
                                    pipeline_status["latest_message"] = log_message
                            ...
                            chunk_dps = structural_dps or self.chunking_func(
                                self.tokenizer,
                                status_doc.content,
                                split_by_character,
                            ...
                            chunks: dict[str, Any] = {
                                compute_mdhash_id(
                                    dp.get("structural_id") or dp["content"],
                                    prefix="chunk-",
                                ): {
                                    **dp,
                                    "full_doc_id": doc_id,
                            ...
                            """
                            CUSTOM ENTITY AND RELATIONSHIP EXTRACTION
                            """
    ...
    async def aretrieve_context(
        self,
        query: str,
        param: QueryParam = QueryParam(mode="mix"),
    ) -> dict[str, Any]:
        """Retrieve structured context without generating an answer."""
        global_config = asdict(self)
        param.original_query = query

        if param.mode in ["local", "global", "hybrid", "mix"]:
            response = await kg_retrieve(
                query.strip(),
                self.chunk_entity_relation_graph,
                self.entities_vdb,
                self.relationships_vdb,
            ...
            raise ValueError(f"Mode {param.mode} does not perform retrieval")

        await self._query_done()
        return response

    async def aexpand_navigation_context(
        self,
        *,
        ...
        """Expand a retrieved chunk/block to a small structural navigation window."""
        if not chunk_id and not block_id:
            raise ValueError("Either chunk_id or block_id must be provided")
        ...
        if chunk_id:
            ...
            raw_chunk_metadata = chunk.get("chunk_metadata")
            chunk_metadata = raw_chunk_metadata if isinstance(raw_chunk_metadata, dict) else {}
            center_block_ids = [str(value) for value in chunk_metadata.get("block_ids") or []]
            artifact_manifest_path = artifact_manifest_path or chunk.get("artifact_manifest_path")

        if block_id:
            center_block_ids = [block_id]

        if not center_block_ids:
            raise ValueError("No block ids were available for navigation expansion")
        if not artifact_manifest_path:
            raise ValueError("artifact_manifest_path is required to resolve block navigation")

        manifest = load_structural_manifest(self.working_dir, artifact_manifest_path)
        response = expand_navigation_context(
            manifest,
            center_block_ids=center_block_ids,
            window=window,
            include_parent_section=include_parent_section,
        )
        response["request"] = {
            "chunk_id": chunk_id,
            "block_id": block_id,
