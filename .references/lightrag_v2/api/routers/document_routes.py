...
from lightrag.structural_chunking import (
    build_docling_blocks,
    build_llm_refined_sections,
    build_structural_artifacts,
    build_sections_from_blocks,
    docling_document_to_dict,
    is_docling_structural_extension,
    parse_docling_toc_candidates,
    should_refine_sections_with_llm,
)
...
_docling_preflight_lock = asyncio.Lock()


async def _append_pipeline_message(message: str) -> None:
    """Append an operator-facing message to the shared pipeline status."""
    logger.info(message)
    try:
        ...
        logger.debug(f"Unable to append pipeline status message: {e}")


def _docling_preflight_sync(install_if_missing: bool) -> None:
    """Ensure Docling is importable and converter can initialize."""
    ...
    # Warm the converter at startup so first upload avoids cold-start overhead.
    DocumentConverter()


async def ensure_docling_ready(
    *, install_if_missing: bool = True, context: str = "runtime"
) -> bool:
    """Run one-time Docling preflight and cache readiness state."""
    global _docling_ready
    if _docling_ready:
        return True

    async with _docling_preflight_lock:
        if _docling_ready:
            return True
        try:
            await asyncio.to_thread(_docling_preflight_sync, install_if_missing)
            _docling_ready = True
            ready_msg = f"Docling preflight ready ({context})"
            logger.info(ready_msg)
            await _append_pipeline_message(ready_msg)
            return True
        except Exception as e:
            error_msg = f"Docling preflight failed ({context}): {e}"
            logger.warning(error_msg, exc_info=True)
            await _append_pipeline_message(error_msg)
            return False


def _format_structural_artifact_message(
    file_path: Path, metadata: dict[str, Any] | None, *, fallback: bool = False
) -> str:
    if not metadata:
        return f"Structured artifact manifest unavailable for {file_path.name}"

    label = "Fallback flat artifact manifest" if fallback else "Docling artifact manifest"
    return (
        f"{label} written for {file_path.name}: "
        f"{metadata.get('artifact_manifest_path', 'unknown manifest')} "
        f"({metadata.get('block_count', 0)} blocks, "
        f"{metadata.get('section_count', 0)} sections, "
        f"{metadata.get('page_count', 0)} pages, "
        f"status={metadata.get('structure_status', 'unknown')})"
    )


async def _extract_docling_supported_file(
    rag: LightRAG, file_path: Path, file: bytes
) -> tuple[str, str | None, dict[str, Any] | None]:
    await _append_pipeline_message(
        f"Docling structural extraction started: {file_path.name}"
    )
    try:
        content, doc_id, metadata = await _extract_with_docling_artifacts(rag, file_path)
        await _append_pipeline_message(
            f"Docling Markdown extraction completed: {file_path.name} "
            f"({len(content)} characters)"
        )
        await _append_pipeline_message(
            _format_structural_artifact_message(file_path, metadata)
        )
        return content, doc_id, metadata
    except Exception as e:
        logger.warning(
            f"Docling structural extraction failed for {file_path.name}; falling back to flat extraction: {e}"
        )
        await _append_pipeline_message(
            f"Docling structural extraction failed for {file_path.name}; "
            f"falling back to flat extraction: {e}"
        )
        content, doc_id, metadata = _extract_docling_fallback_artifacts(
            rag, file_path, file, e
        )
        if metadata:
            await _append_pipeline_message(
                _format_structural_artifact_message(
                    file_path, metadata, fallback=True
                )
            )
        else:
            await _append_pipeline_message(
                f"Fallback flat extraction produced no content for {file_path.name}"
            )
        return content, doc_id, metadata


# CUSTOM STRUCTURAL CHUNKING:
# These helpers are the Docling-aware extraction bridge for Phase 4 structural
# chunking. The original route flattened Docling output to Markdown and handed
# only text to LightRAG. The custom path keeps Markdown for compatibility, but
# also writes a durable artifact manifest plus compact metadata so downstream
# chunking can preserve sections, tables, figures, and citation links.
async def _extract_with_docling_artifacts(
    rag: LightRAG, file_path: Path
) -> tuple[str, str, dict[str, Any]]:
    if not await ensure_docling_ready(
        install_if_missing=True, context=f"upload:{file_path.name}"
    ):
        raise RuntimeError("Docling is unavailable")
    from docling.document_converter import DocumentConverter  # type: ignore
    ...
    raw_docling = docling_document_to_dict(result.document)
    blocks = build_docling_blocks(raw_docling)
    ...
    if should_refine_sections_with_llm(blocks, sections, toc_candidates):
        try:
            sections = await build_llm_refined_sections(
                blocks=blocks,
            ...
            warnings.append(f"LLM section refinement failed: {e}")
            logger.warning(
                f"LLM section refinement failed for {file_path.name}; "
                "using heuristic sections",
                exc_info=True,
            )
    metadata = build_structural_artifacts(
        content=content,
...
# CUSTOM STRUCTURAL CHUNKING:
# If Docling fails on a supported file type, ingestion should still succeed.
# This function falls back to the old flat extraction behavior and writes an
# error manifest so operators and retrieval citations can explain why the
# document did not receive full structural treatment.
def _extract_docling_fallback_artifacts(
    rag: LightRAG, file_path: Path, file: bytes, error: Exception
) -> tuple[str, str | None, dict[str, Any] | None]:
    content = _extract_flat_supported_file(file_path.suffix.lower(), file)
    if not content:
        return "", None, None
    ...
    metadata = build_structural_artifacts(
        ...
        working_dir=rag.working_dir,
        doc_id=doc_id,
        extraction_error=f"Docling structural extraction failed: {error}",
    )
    return content, doc_id, metadata


# CUSTOM STRUCTURAL CHUNKING:
# This wraps the pre-existing non-Docling extraction logic in one reusable
# helper. It is used both when flat extraction is explicitly configured and when
# Docling fails, which keeps fallback behavior compatible with the old route.
def _extract_flat_supported_file(ext: str, file: bytes) -> str:
    content = ""
    if ext == ".pdf":
...
async def pipeline_enqueue_file(rag: LightRAG, file_path: Path) -> bool:
    ...
    try:
        ...
        file = None
        await _append_pipeline_message(
            f"Upload received for processing: {file_path.name} ({ext or 'unknown extension'})"
        )
        async with aiofiles.open(file_path, "rb") as f:
            file = await f.read()
        await _append_pipeline_message(
            f"Preparing extraction stage for {file_path.name}"
        )
        ...
        match ext:
            ...
            case ".pdf":
                if global_args.document_loading_engine == "DOCLING":
                    await _append_pipeline_message(
                        f"Starting Docling extraction for {file_path.name}"
                    )
                    # CUSTOM STRUCTURAL CHUNKING:
                    # Docling-supported uploads now create an artifact manifest
                    # and enqueue compact structure metadata instead of passing
                    # only flattened Markdown into the RAG queue.
                    content, doc_id, document_metadata = (
                        await _extract_docling_supported_file(rag, file_path, file)
                    )
                else:
                    await _append_pipeline_message(
                        f"Starting flat PDF extraction for {file_path.name}"
                    )
                    content = _extract_flat_supported_file(ext, file)
            case ".docx":
                if global_args.document_loading_engine == "DOCLING":
                    await _append_pipeline_message(
                        f"Starting Docling extraction for {file_path.name}"
                    ...
                    # CUSTOM STRUCTURAL CHUNKING:
                    # Keep DOCX structure when Docling is configured, while
                    # still falling back to the old parser if structural
                    # extraction fails for this particular file.
                    content, doc_id, document_metadata = (
                        await _extract_docling_supported_file(rag, file_path, file)
                    )
                else:
                    await _append_pipeline_message(
                        f"Starting flat DOCX extraction for {file_path.name}"
                    )
                    content = _extract_flat_supported_file(ext, file)
            case ".pptx":
                if global_args.document_loading_engine == "DOCLING":
                    ...
                    # CUSTOM STRUCTURAL CHUNKING:
                    # PPTX files can contain slide-level structure. The
                    # artifact manifest gives later chunks a stable citation
                    # target instead of only a flat text blob.
                    content, doc_id, document_metadata = (
                        await _extract_docling_supported_file(rag, file_path, file)
                    )
                else:
                    await _append_pipeline_message(
                        f"Starting flat PPTX extraction for {file_path.name}"
                    )
                    content = _extract_flat_supported_file(ext, file)
            case ".xlsx":
                if global_args.document_loading_engine == "DOCLING":
                    ...
                    # CUSTOM STRUCTURAL CHUNKING:
                    # XLSX table-like content benefits from row-aware splitting
                    # later in the pipeline, so keep structural metadata when
                    # Docling is the configured loader.
                    content, doc_id, document_metadata = (
                        await _extract_docling_supported_file(rag, file_path, file)
                    )
                else:
                    await _append_pipeline_message(
                        f"Starting flat XLSX extraction for {file_path.name}"
                    )
                    content = _extract_flat_supported_file(ext, file)
            case _:
                logger.error(
        ...
        if content:
            ...
            # CUSTOM STRUCTURAL CHUNKING:
            # Structural metadata is optional so text-only paths keep working.
            # When present, the explicit doc ID keeps the artifact directory,
            # doc-status record, and future chunk IDs deterministic.
            if (
                doc_id is None
                and document_metadata is None
                and global_args.document_loading_engine == "DOCLING"
                and is_docling_structural_extension(ext)
            ):
                doc_id = compute_mdhash_id(content, prefix="doc-")
            ...
            await rag.apipeline_enqueue_documents(
                ...
                overwrite_existing=bool(document_metadata),
            )
            await _append_pipeline_message(
                f"File enqueued for processing: {file_path.name}"
            )
            return True
        else:
