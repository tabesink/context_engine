...
When the selected `lightrag_domain_id` is registered in `LIGHTRAG_DOMAIN_REGISTRY`, admin upload forwarding is delegated to the registered LightRAG service and the local response may have no local indexing `job_id`.
