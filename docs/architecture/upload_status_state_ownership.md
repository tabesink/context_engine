...
- `jobs.status` owns active operation state during the compatibility period:
  `queued`, `running`, `succeeded`, `failed`, or `canceled`.
- `jobs.stage` owns the current processing phase, such as `queued`, `parsing`,
  `indexing_lightrag`, `waiting_remote`, `complete`, or `failed`.
...
- `GET /admin/lightrag/domains/{domain_id}/processing-status` summarizes domain
  processing activity.
- `GET /jobs` and `GET /jobs/{job_id}` are admin diagnostics during the
