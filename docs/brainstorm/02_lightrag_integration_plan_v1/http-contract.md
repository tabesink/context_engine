...
Settings are loaded via Pydantic (`app/core/config.py`); env names above map to `lightrag_*` fields (e.g. `LIGHTRAG_TIMEOUT_SECONDS` → `lightrag_timeout_seconds`, parsed as a float).
...
Mode mapping (`LightRAGRemoteAdapter._lightrag_mode`):

- `semantic` -> `mix`
- `hybrid` -> `mix`
- `auto` -> `mix`
- `navigation` -> local Context Engine navigation path; the adapter still exposes `naive` for non-semantic modes but normal `RetrievalService` routing never calls the remote adapter for `navigation` when LightRAG is enabled.
...
The adapter normalizes chunks and references into local `Evidence`. Each `document_id` in evidence is a stable UUID: valid UUID strings pass through; any other upstream id is hashed to **UUID v5** (`LightRAGRemoteAdapter._document_uuid`) so response shapes stay uniform.
